  ...
  @ViewChildren('rgLastInitial') lastInitialItems: QueryList<ElementRef>;
    ...
    private initializeFacetFamilies() {
        ...
        */
        ...
        this.facetFamilies[BioFilterFamilyType.Category].isAllowedToBeShown = false; // for ScienceMakers, always hidden
    ...
    getHistoryMakers(givenPage: number, givenPageSize: number, filterSpecToUse: string) {
        ...
        if (this.myBornThisTimeFilterFlag) {
            this.historyMakerService.getHistoryMakersBornThisWeek(givenPage, givenPageSize, genderFacetSpec,
            ...
            // The only use for biography search fields is with a search, not a born this week, so only bother
    ...
    private initTitleForPage() {
        ...
        if (!this.cardView && !this.textView) {
            if (this.totalBiographiesFound > 0) {
                ...
                this.biographySetTitle = "Map view featuring 0 ScienceMakers";
                this.screenReaderSummaryTitle = "Map view of empty ScienceMakers set";
            }
        }
        else {
            ...
            if (countReturned > 0) {
                ...
                if (this.myCurrentPage != 1) {
                    this.biographySetTitle = "No ScienceMakers for page " + this.myCurrentPage + " (" + this.myCurrentPageSize + " per page)";
                }
                else {
                    if (this.myBornThisTimeFilterFlag)
                ...
                this.screenReaderSummaryTitle = "Empty ScienceMakers set";
            }
        }
        ...
        // such as "ScienceMakers Biographies Set", so use the detailed title with suffix indicating "Maker Directory".
    ...
    private initializeTitleAndPaging(givenPage: number, givenPageSize: number, totalCount: number,
      titleLabelSuffixSoFar: string) {
        ...
        // For some sets, have total be X ScienceMakers Found; for others, just X ScienceMakers
        ...
        if (totalCount == 1)
            this.totalBiographiesFoundSuffix = "1 ScienceMaker";
        else
            this.totalBiographiesFoundSuffix = totalCount.toLocaleString() + " ScienceMakers";
